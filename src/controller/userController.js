...
let registerUser = async (req, res) => {
    ...
    if (!name || !email || !password) {
        return res.status(401).json({ message: 'Need to fill all the required fields' });
    }
    try {
        if (name.length < 4) {
            return res.status(401).json({ message: 'Name should have atleast 4 characters' });
        }
        ...
        return res.status(200).json({ message: 'User created successfully', user: user.rows[0] });
    } catch (err) {
